...
const Sidebar: React.FC = () => {
    ...
    const [editWidget, setEditWidget] = useState<{ edit: boolean; widgetKey: WidgetEnum | null }>({
        edit: false,
        widgetKey: null
    });
    ...
    return (
        <>
            <div className={`sidebar ${editMode ? "edit-mode" : ""}`}>
                {editMode && <div className="widget-menu-title">
                                <div><IoMdArrowDropright /> Widget menu</div>
                            </div>}
                {Widgets && editMode &&
